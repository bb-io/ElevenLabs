...
[ActionList("Dubbing")]
public class DubbingActions(InvocationContext invocationContext, IFileManagementClient fileManagementClient) : ElevenLabsInvocable(invocationContext)
{
    ...
    public async Task<CreateDubResponse> CreateDub([ActionParameter] CreateDubRequest input)
    {
        ...
        {
            var csvStream = await fileManagementClient.DownloadAsync(input.CsvFile);
            request.AddFile("csv_file", () => csvStream, input.CsvFile.Name, "text/csv");

            if (input.ForegroundAudio != null)
            {
                var fg = await fileManagementClient.DownloadAsync(input.ForegroundAudio);
            ...
            {
                var bg = await fileManagementClient.DownloadAsync(input.BackgroundAudio);
